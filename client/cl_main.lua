...
AddEventHandler("Dox-hunting:payammo", function()
  ...
  if DoxCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and not DoxCore.Shared.Weapons["WEAPON_SNIPERRIFLE2"] then
    TriggerServerEvent("remove:money")
  else
    DoxCore.Functions.Notify("How am Im Suppose to reload your sniper if your not holding it?", "error")
  end
end)
...
AddEventHandler('Dox-hunting:skinAnimal', function()
  DoxCore.Functions.TriggerCallback('DoxCore:HasItem', function(hasItem)
    if hasItem then
      if DoesEntityExist(baitAnimal) then
      ...
      DoxCore.Functions.Notify("You need a knife", "error")
    end
  end, "WEAPON_KNIFE")
end)
...
AddEventHandler('Dox-hunting:usedBait', function()
  for k,v in pairs (HuntingZones) do
    ...
    --  TriggerEvent("DoxCore:Notify", "UHMMM FIND SOMETHING TO WRITE HERE",2)
    end
  end
end)
